...
FILE_GRAPH_BOX="example_boxplot.png"
FILE_GRAPH_SCATTER="example_scatterplot.png"
...
pv056-evaluate-features -c configs/fs/default.json -di "$FILE_DATASET_SPLIT"  -do "$FILE_DATASET_FS" > "logs/log-fs-$TIMESTAMP_FS.log"
now
printf "%s - FS done.\n\n" "$NOW"
...
pv056-apply-od-methods -c configs/od/default.json -di "$FILE_DATASET_FS" -do "$FILE_DATASET_OD" > "logs/log-od-$TIMESTAMP_OD.log"
now
printf "%s - OD done.\n\n" "$NOW"
...
pv056-run-clf -c configs/clf/default.json -di "$FILE_DATASET_RM" -do "$FILE_DATASET_CLF" > "logs/log-clf-$TIMESTAMP_CLF.log"
now
printf "%s - CLF done.\n\n" "$NOW"
